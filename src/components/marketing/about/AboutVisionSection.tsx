...
export function AboutVisionSection({ className }: AboutVisionSectionProps) {
    return (
        <section aria-label="Vision artistique" className={cn('relative overflow-hidden bg-(--bg-primary) py-18 sm:py-24 lg:py-28', className)}>
            <div aria-hidden="true" className="pointer-events-none absolute right-0 top-10 h-60 w-60 rounded-full bg-white/4 blur-3xl" />
            <div aria-hidden="true" className="pointer-events-none absolute left-1/2 top-20 h-24 w-[18rem] -translate-x-1/2 rounded-full bg-(--accent)/8 blur-3xl" />

            <Container className="relative z-10">
                <div className="grid gap-12 lg:grid-cols-[minmax(0,0.86fr)_minmax(0,1.14fr)] lg:gap-16">
                    <div className="lg:sticky lg:top-28 lg:self-start">
                        <p className="text-[11px] uppercase tracking-[0.32em] text-white/38">Vision</p>
                    ...
                    <div className="grid gap-4">
                        {visionFragments.map((fragment, index) => (
                            <article
                                key={fragment.index}
                                className={cn(
                                    'group rounded-3xl border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.04),rgba(255,255,255,0.02))] px-5 py-5 transition-transform duration-500 sm:px-6 sm:py-6',
                                    index % 2 === 1 && 'lg:ml-10',
                                )}
                            >
                                <div className="flex items-start gap-4">
                                    <span className="mt-1 text-[10px] uppercase tracking-[0.22em] text-white/30">{fragment.index}</span>

                                    <div className="min-w-0 flex-1">
                                        <div className="flex items-center gap-3">
                                            <p className="text-[10px] uppercase tracking-[0.22em] text-white/42">{fragment.label}</p>
