...
function OeuvreLightbox({ images, title, activeIndex, isOpen, onClose, onPrev, onNext }: OeuvreLightboxProps) {
        ...
        <div className="fixed inset-0 z-100 bg-[rgba(3,8,16,0.94)] backdrop-blur-md" role="dialog" aria-modal="true" aria-label={`Agrandissement de ${title}`}>
            <div className="absolute right-4 top-4 z-10">
                <button
                    type="button"
                    onClick={onClose}
                    className="inline-flex cursor-pointer h-11 w-11 items-center justify-center rounded-full border border-white/10 bg-white/6 text-white transition-colors duration-300 hover:bg-white/12"
                    aria-label="Fermer"
                >
                    ✕
                </button>
                    ...
                    {images.length > 1 && (
                        <>
                            <button
                                type="button"
                                onClick={onPrev}
                                className="absolute cursor-pointer left-3 top-1/2 inline-flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full border border-white/10 bg-[rgba(6,12,21,0.7)] text-white transition-colors duration-300 hover:bg-[rgba(6,12,21,0.9)]"
                            ...
                            <button
                                type="button"
                                onClick={onNext}
                                className="absolute cursor-pointer right-3 top-1/2 inline-flex h-11 w-11 -translate-y-1/2 items-center justify-center rounded-full border border-white/10 bg-[rgba(6,12,21,0.7)] text-white transition-colors duration-300 hover:bg-[rgba(6,12,21,0.9)]"
                ...
                <Container>
                    <div className="mb-10 max-w-2xl">
                        ...
                        <Heading level={2} className="mt-4 text-white">
                            Détails, matière, autres présences.
                        </Heading>

                        <Text variant="muted" className="mt-4 text-white/70">
                            Quelques vues pour approcher le geste de plus près.
                        </Text>
                    </div>
                    ...
                    <div className="grid gap-4 grid-cols-3 sm:grid-cols-3">
                        {images.map((image, index) => (
                            <button
                                ...
                                className="group relative overflow-hidden rounded-[1.35rem] border border-white/10 bg-white/3 text-left cursor-pointer"
