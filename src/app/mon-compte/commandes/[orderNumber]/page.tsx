    ...
    return (
        <main className="min-h-screen bg-(--bg-primary) px-5 py-8 text-white sm:px-8">
            <div className="mx-auto max-w-6xl">
                ...
                <header className="mt-10 grid gap-6 border-b border-white/12 pb-8 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-end">
                    <div>
                        <p className="text-xs font-semibold uppercase tracking-[0.28em] text-white/42">Commande</p>
                        <h1 className="mt-4 font-(family-name:--font-heading) text-5xl leading-none sm:text-7xl">{order.orderNumber}</h1>
                        <p className="mt-4 max-w-2xl text-base leading-7 text-white/62">
                            Commande du {formatOrderDate(order.createdAt)} · {getOrderStatusLabel(order.status)}
                        </p>
                    </div>
